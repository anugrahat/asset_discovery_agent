...
class DrugAssetDiscoveryAgent:
    ...
    def __init__(self, api_key: str = None, llm_api_key: str = None):
        ...
        self.asset_webcrawler = AssetWebCrawler()
    ...
    async def analyze_disease_failures(
        ...
        # 4. Web crawling for shelved/discontinued drugs
        web_candidates = []
        try:
            web_results = await self._discover_global_assets(disease=disease)  # uses include_press_releases=True
            # Flatten to candidate-like rows
            for source, assets in (web_results or {}).items():
                for a in assets:
                    drug_name = (a.get("drug_name") or a.get("drug") or "").strip()
                    if not drug_name:
                        continue
                    if any(c.get('drug_name') == drug_name for c in (ct_candidates + terminated_candidates)):
                        continue
        ...
        drug_name_map = {c['drug'].lower(): c for c in enriched_candidates}
        ...
        for candidate in enriched_candidates:
            ...
            if drug_name:
                ...
                # If not found, try with drug resolver normalized name
                if not ob_match and not pb_match:
                    # Check if we already have resolver data from ChEMBL enrichment
                    normalized_name = None
                    if candidate.get("pref_name"):
                        normalized_name = candidate["pref_name"]
                    elif candidate.get("llm_normalized_name"):
                        normalized_name = candidate["llm_normalized_name"]
                    
                    # If no existing normalized name, resolve it
                    if not normalized_name:
                        try:
                            resolver_result = await self.drug_resolver.resolve_drug(drug_name)
                            if resolver_result:
                                normalized_name = (resolver_result.get("pref_name") or 
                                                 resolver_result.get("llm_normalized_name") or
                                                 resolver_result.get("normalized_name"))
                        except Exception:
                            pass
                ...
                try:
                    ...
                    if asset_owner_info and asset_owner_info.get("asset_owner"):
                        ...
                        if regional_info:
                            ...
                            # Format for display - show approved regions
                            approved_regions = []
                            if regional_info.get('fda'): approved_regions.append('FDA')
                            if regional_info.get('ema'): approved_regions.append('EMA')
                            if regional_info.get('pmda'): approved_regions.append('PMDA')
                            if regional_info.get('nmpa'): approved_regions.append('NMPA')
                            if regional_info.get('health_canada'): approved_regions.append('HC')
                            if regional_info.get('cdsco'): approved_regions.append('CDSCO')
                            candidate['regional_approvals_display'] = ', '.join(approved_regions) if approved_regions else 'None'
                        
                        # For low confidence LLM results or "unknown", prefer clinical trials sponsor data
                        if (confidence == "low" or llm_owner == "unknown") and candidate.get("sponsor") and candidate.get("sponsor") != "Unknown":
                            originator_company = candidate.get("sponsor")
                            # Use simple ownership chain with clinical trials sponsor
                            candidate["ownership_chain"] = candidate.get("sponsor", "")
                            logger.debug(f"Using clinical trials sponsor over {confidence}-confidence LLM for {drug_name}")
                        elif llm_owner != "unknown":
...
"""
    ...
    async def _get_llm_asset_owner(self, drug_name: str) -> Optional[Dict]:
        ...
        try:
            prompt = f"""You are a pharmaceutical research analyst. Find ONLY VERIFIED information for '{drug_name}'.

CRITICAL RULES:
1. Use ONLY information you can verify from reliable sources
2. If you cannot find verified information, return "unknown" 
3. Do NOT guess or infer ownership transfers or approvals
4. Distinguish between trial sponsors vs actual asset owners
5. Only include companies with confirmed IP/development rights

For '{drug_name}', find:
- Current asset owner (company with IP rights)
- Original developer (if verifiable)
- Ownership transfers (only if documented)
- Regional drug approvals (FDA, EMA, PMDA, NMPA, Health Canada, DCGI India)

If ownership history or approvals are unclear or unverified, use simplified format.
...
{{
  ...
  "confidence": "high/medium/low"
    ...
    def _calculate_high_potential_score(self, candidate: Dict) -> float:
        ...
        # Weights (without regional approvals)
        w_phase = 0.25
        w_disc = 0.25  
        w_rec = 0.20
        w_avail = 0.15
